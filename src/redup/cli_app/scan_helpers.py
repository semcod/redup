...
def print_scan_header(
    path: Path, ext_list: list[str], min_lines: int, min_similarity: float
) -> None:
...
def print_scan_summary(dup_map: DuplicationMap) -> None:
    ...
    typer.echo(
        f"📊 Scanned {dup_map.stats.files_scanned} files "
        f"({dup_map.stats.total_lines} lines, "
        f"{dup_map.stats.scan_time_ms:.0f}ms)"
    )
    typer.echo(
        f"Found {dup_map.total_groups} duplicate groups "
