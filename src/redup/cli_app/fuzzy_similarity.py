...
def _analyze_blocks_with_detectors(
    html_css_blocks: list, other_blocks: list, threshold: float
) -> list:
    ...
    if other_blocks:
        ...
        typer.echo(
            f"    ✨ Found {len(universal_similar)} similar component pairs from other languages"
        )
...
def _report_fuzzy_results(similar_pairs: list) -> None:
    ...
    if similar_pairs:
        ...
        typer.echo(
            f"  📊 Language groups: {len(set(Path(b1.file).suffix + '+' + Path(b2.file).suffix for b1, b2, _ in similar_pairs))}"
        )
