...
class DuplicateTaskExporter:
    ...
    def _build_description(
        ...
        if len(packages) > 2:
            lines.append("Extract to shared library due to cross-package usage.")
        elif group.total_lines > 50:
            lines.append("Consider extracting to a new module.")
        elif group.fragments[0].function_name:
            lines.append("Extract function to a shared utility module.")
        else:
            lines.append("Extract code block to eliminate duplication.")
    ...
    def _render_todo_md(self) -> str:
        ...
        lines.append("")
        lines.append("Generated by [redup](https://github.com/semcod/redup)")
        ...
        for priority in ["critical", "major", "minor"]:
            ...
            for i, task in enumerate(tasks, 1):
                checkbox = "- [ ]" if task.task_id is None else "- [x]"
        ...
        lines.append("## Planfile Export Configuration")
        lines.append("")
        lines.append("```yaml")
        lines.append("# Add to your planfile.yaml to sync these tasks:")
        lines.append("tasks:")
