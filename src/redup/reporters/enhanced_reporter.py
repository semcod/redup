...
class EnhancedReporter:
    ...
    def _get_language_metrics(self) -> dict[str, Any]:
        ...
        lang_stats = defaultdict(
            lambda: {"files": set(), "groups": 0, "dup_lines": 0, "fragments": 0}
        )
    ...
    def _get_file_metrics(self) -> dict[str, Any]:
        ...
        file_stats = defaultdict(lambda: {"groups": 0, "fragments": 0, "dup_lines": 0})
        ...
        sorted_files = sorted(file_stats.items(), key=lambda x: x[1]["dup_lines"], reverse=True)

        return {
            "total_files_with_dups": len(file_stats),
            "top_files": sorted_files[:10],
            "avg_dups_per_file": sum(s["dup_lines"] for s in file_stats.values())
            / max(len(file_stats), 1),
        }
