...
def _format_communities(communities: list[dict]) -> str:
    ...
    for c in communities[:10]:
        ...
        lines.append(f"- **{name}**: {loc} LOC, {len(members)} members, avg sim={sim:.2f}\n")
    return "".join(lines)
    ...
    try:
        response = litellm.completion(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are a code refactoring expert. Return only valid JSON.",
                },
...
def format_plan_markdown(plan: RefactorPlan) -> str:
    ...
    lines.append("")
    lines.append(
        f"*Generated by `{plan.model}` in {plan.duration_ms:.0f}ms "
