...
from redup.core.hasher import HashedBlock, HashIndex, _blocks_from_different_locations
...
def _create_duplicate_group(
    hash_val: str, blocks: list[HashedBlock], duplicate_type: DuplicateType
) -> DuplicateGroup:
    ...
    normalized_name = (
        fragments[0].function_name if fragments and fragments[0].function_name else None
    )
