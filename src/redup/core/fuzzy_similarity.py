...
class HTMLComponentExtractor:
    ...
    def _normalize_html(self, html: str) -> str:
        ...
        html = re.sub(
            r'class\s*=\s*["\']([^"\']+)["\']',
            lambda m: f'class="{self._normalize_class_name(m.group(1))}"',
            html,
        )
    ...
    def _extract_attributes(self, html: str) -> dict[str, str]:
        ...
        button_types = re.findall(
            r'(?:button|input)[^>]*type\s*=\s*["\']([^"\']+)["\']', html, re.IGNORECASE
        )
        if button_types:
...
class FuzzySimilarityDetector:
    ...
    def _compute_attribute_similarity(
        self, attrs1: dict[str, str], attrs2: dict[str, str]
    ) -> float:
