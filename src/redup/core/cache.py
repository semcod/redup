...
class HashCache:
    ...
    def store_file_hashes(
        self, file_path: Path, content: str, block_hashes: dict[tuple[int, int], tuple[str, str]]
    ) -> None:
        ...
        try:
            ...
            # Store block hashes
            self.db.execute("DELETE FROM block_hashes WHERE file_path=?", (str(file_path),))
    ...
    def cleanup_old_entries(self, max_age_days: int = 30) -> None:
        ...
        try:
            ...
            self.db.execute("DELETE FROM file_hashes WHERE last_scan_time < ?", (cutoff_time,))
...
def build_hash_index_with_cache(
    ...
    """
    from redup.core.hasher import HashedBlock, HashIndex
